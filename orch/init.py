...
to roughly a page each. All projects share the VM filesystem, so for anything
...
cap: the prompt reaches the target agent over stdin, so a long request will be
delivered, it just makes for a worse brief.

If a bridge comes back `[permanent]` saying the request could not be delivered,
resubmitting the same bytes cannot help \u2014 shorten it, move the bulk into a
